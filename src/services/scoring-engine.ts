import { Task, UserMetrics, TaskScore, ScoringWeights, DEFAULT_WEIGHTS, TIME_WINDOWS } from '../types';
...
export class ScoringEngine {
   ...
   */
  public urgencyContribution(task: Task, metrics: UserMetrics): number {
    const { category } = task;
   ...
   */
  public inverseEffort(effortMinutes: number): number {
   ...
   */
  public timeOfDayFactor(timeGate: string | undefined, currentTime: string): number {
    if (!timeGate) {
      return 1; // No time gate, always eligible
    }

    const timeWindow = timeGate as keyof typeof TIME_WINDOWS;
    ...
    const windowStart = this.parseTime(TIME_WINDOWS[timeWindow].start);
    const windowEnd = this.parseTime(TIME_WINDOWS[timeWindow].end);
   ...
   */
  public calculateScore(task: Task, metrics: UserMetrics, currentTime: string): TaskScore {
    ...
    const score = (
      this.weights.W_urgency * urgency +
   ...
   */
  private getUrgencyReason(task: Task, metrics: UserMetrics): string {
   ...
   */
  private extractHourMinute(isoDateTime: string): { hour: number; minute: number } {
